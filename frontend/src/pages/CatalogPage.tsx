...
export function CatalogPage() {
  ...
  return <section className="page">
    ...
    <div className="catalog-grid">{query.data?.content.map((product) => { const certificateMatches = selectedCertificate && (selectedCertificate.targetCategory === 'ANY' || selectedCertificate.targetCategory === product.category); const discount = certificateMatches ? Math.min(Number(product.price), Number(selectedCertificate.discountAmount)) : 0; const total = Math.max(0, Number(product.price) - discount); return <article className="product" key={product.id}><div className="product-icon">{product.category[0]}</div><div className="product-body"><span>{product.category} · {Number(cashbackFor(product.category)).toFixed(0)}% cashback</span><h3>{product.name}</h3><p>{product.merchantName}</p></div><div className="earn-preview">≈ {Math.round(total * Number(cashbackFor(product.category)) / 100)} базовых баллов после скидки</div>{selectedCertificate && !certificateMatches ? <div className="price-breakdown"><span>Выбран сертификат для {selectedCertificate.targetCategory}</span><span>К этому товару не применится</span></div> : null}{discount > 0 ? <div className="price-breakdown"><span>Цена {Number(product.price).toFixed(0)} ₽</span><span>Сертификат -{discount.toFixed(0)} ₽</span></div> : null}<div className="product-bottom"><b>{total.toFixed(2)} ₽</b>{customer ? <button className="primary small" disabled={buy.isPending || Boolean(selectedCertificate && !certificateMatches)} onClick={() => buy.mutate({ merchantId: product.merchantId, productId: product.id })}>Купить</button> : <NavLink className="secondary-link small" to="/login">Войти</NavLink>}</div></article>; })}</div>
  </section>;
}
