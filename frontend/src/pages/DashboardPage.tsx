...
export function DashboardPage() {
  ...
  const preferredCategory = profileQuery.data?.favoriteCategories[0]?.category;
  const redeem = useMutation({ mutationFn: () => redeemReward(redeemPoints, preferredCategory ? `${preferredCategory} certificate` : 'Shopping discount certificate', preferredCategory), onSuccess: () => { client.invalidateQueries({ queryKey: ['balance'] }); client.invalidateQueries({ queryKey: ['transactions'] }); client.invalidateQueries({ queryKey: ['certificates'] }); } });
  ...
  return <section className="page">
    ...
    <div className="two-columns">
      <div className="card redeem-card"><div className="card-head"><h2>Обменять баллы</h2><span>10 баллов = 1 ₽</span></div><p className="muted">Баллы превращаются в сертификат для категории, которую вы чаще всего покупаете. Так скидка тоже основана на истории покупок.</p><input type="range" min="50" max={maxRedeem} step="10" value={Math.min(redeemPoints, maxRedeem)} onChange={(event) => setRedeemPoints(Number(event.target.value))} /><div className="redeem-summary"><b>{Math.min(redeemPoints, maxRedeem)} баллов</b><span>сертификат {preferredCategory ?? 'ANY'} на {(Math.min(redeemPoints, maxRedeem) / 10).toFixed(0)} ₽</span></div><button className="primary" disabled={redeem.isPending || (balanceQuery.data?.pointsBalance ?? 0) < 50} onClick={() => redeem.mutate()}>{redeem.isPending ? 'Оформляем...' : 'Создать сертификат'}</button>{redeem.data ? <div className="alert success">Сертификат {redeem.data.confirmationCode} для {redeem.data.targetCategory} создан. {redeem.data.personalizationReason}</div> : null}{redeem.isError ? <div className="alert error">Недостаточно баллов для списания.</div> : null}</div>
